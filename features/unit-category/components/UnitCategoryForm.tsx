...
import { useCreateUnitCategory, useUpdateUnitCategory } from '@/features/unit-category/hooks/useUnitCategories';
...
interface UnitCategoryFormProps {
...
export function UnitCategoryForm({ unitCategory, onSuccess, onCancel, formId, hideActions, onErrorChange, onSubmittingChange }: UnitCategoryFormProps) {
  const [name, setName] = useState(unitCategory?.name ?? '');
  const [description, setDescription] = useState(unitCategory?.description ?? '');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (unitCategory) {
        await updateMutation.mutateAsync({ id: unitCategory.id, dto: { name, description } });
        handleApiSuccess('Unit category updated successfully');
      } else {
      ...
      const message = handleApiError(err, {
        defaultMessage: 'Failed to save unit category',
      });
    ...
    <form id={formId} onSubmit={handleSubmit} className="space-y-4">
      ...
      {hideActions ? null : (
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Saving…' : unitCategory ? 'Update' : 'Create'}</Button>
        </div>
      )}
    </form>
  );
